...
## 7) MGT 697 Additional Deliverable Status

The PDF requests scripted user simulation with mild variant preference if MGT 697 applies to the team.

Current status:

- Infrastructure is ready (assignment + event ingestion + rollups).
- Dedicated simulation scripts that generate biased behavior are not yet added in this repo.
- Recommended next addition:
  - add `packages/jobs/src/simulateBiasedTraffic.ts`
  - generate events with slightly higher click/submit probability for one variant
  - verify bias appears in rollups and dashboard metrics over time
...
- `GET /health`
- `POST /v1/assignment` with a fixed `anonymous_user_id` twice and verify stable variants
- `POST /v1/events` and verify accepted count > 0
